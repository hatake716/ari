...
class MainActivity : ComponentActivity() {
    ...
    private fun updateHud() {
        ...
        lastHudKey=hudKey
        dateLabel?.text="${formatDay(c.day)}  ·  ${if(c.winter)"冬ごもり"else if(c.temperature>=24)"夏"else"春・秋"} ${c.temperature.toInt()}℃"
        if(metrics.size==4) {
