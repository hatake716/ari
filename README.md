...
7. エンディングの「新たな女王アリを迎える巣を作る」で、同じ保存枠の建設モードへ進めます。終了済みの保存枠を開き直しても利用できます。旧記録は、新しい巣で「この巣に、女王を迎える」を押すまで残ります。建設を途中でやめても旧記録は失われません。

ピンチで全体表示から最大10倍まで拡大し、ドラッグで断面を移動できます。「全体」で深くなった巣の端まで見渡せます。部屋のタップで生態・役割を読み、「観察記録」で群れの履歴を確認できます。開始後には巣や戦闘を操作するボタンはありません。
...
- 群れの成長に応じて最大48室まで増室。通路→部屋の順に掘り、育児室・貯蔵室・休息室へ群れが広がる様子を表示。完成室数・掘削進捗・群れの総数と記録で成長を追えます。
